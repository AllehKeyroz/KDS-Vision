...
import { PlusCircle, Loader2, Edit, Trash2, MoreVertical, CheckCircle2, Upload, Download, Timer } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { db } from '@/lib/firebase';
import { collection, addDoc, onSnapshot, doc, updateDoc, deleteDoc, serverTimestamp, Timestamp, getDocs } from 'firebase/firestore';
...
import { projectsToCsv, csvToProjects } from '@/lib/csvUtils';
import { saveProjectsToFirestore } from '@/lib/data';

const MiniCountdown: React.FC<{ deadline: Date | undefined }> = ({ deadline }) => {
    ...
    const isExpired = countdown === 'Prazo Expirado';

    return (
        <div className={`text-xs font-mono flex items-center ${isExpired ? 'text-red-500' : 'text-muted-foreground'}`}>
...
export default function ProjectsPage() {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const projectData: Partial<Project> = {
            ...
            deadline: formData.get('deadline') ? Timestamp.fromDate(new Date(formData.get('deadline') as string)) : undefined,
        };
    ...
    const handleExportProjects = () => {
        try {
            ...
            if (link.download !== undefined) { // feature detection
                ...
                link.setAttribute('download', `projetos_completos_${clientId}.csv`);
                link.style.visibility = 'hidden';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                toast({ title: "Projetos Exportados!", description: "Os projetos completos foram exportados para um arquivo CSV." });
            }
        } catch (error) {
            console.error("Error exporting projects:", error);
            toast({ title: "Erro ao Exportar", description: "Não foi possível exportar os projetos completos.", variant: "destructive" });
        }
    };
    ...
    const handleImportProjects = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file) {
            toast({ title: "Nenhum arquivo selecionado", description: "Por favor, selecione um arquivo CSV para importar.", variant: "destructive" });
            return;
        }

        if (file.type !== 'text/csv') {
            toast({ title: "Formato de arquivo inválido", description: "Por favor, selecione um arquivo CSV.", variant: "destructive" });
            return;
        }
        ...
        reader.onload = async (e) => {
            try {
                ...
                const importedProjects = csvToProjects(csvString);

                // Sobrescrever o clientId de todos os projetos importados com o clientId da URL atual
                const projectsToSave = importedProjects.map(project => ({
                    ...project,
                    clientId: clientId, // Usa o clientId da URL
                    sections: project.sections?.map(section => ({
                        ...section,
                        projectId: project.id, // Garante que o projectId da seção esteja correto
                        tasks: section.tasks?.map(task => ({
                            ...task,
                            projectId: project.id, // Garante que o projectId da tarefa esteja correto
                            sectionId: section.id // Garante que o sectionId da tarefa esteja correto
                        }))
                    }))
                }));

                await saveProjectsToFirestore(projectsToSave);
                toast({ title: "Projetos Importados!", description: `${importedProjects.length} projetos (e seus detalhes) foram importados com sucesso.` });
            } catch (error) {
                console.error("Error importing projects:", error);
                toast({ title: "Erro ao Importar", description: "Não foi possível importar os projetos. Verifique o formato do arquivo.", variant: "destructive" });
            } finally {
    ...
    const handleExportSingleProject = (project: Project) => {
        try {
            ...
            if (link.download !== undefined) { // feature detection
                const url = URL.createObjectURL(blob);
                link.setAttribute('href', url);
                link.setAttribute('download', `projeto_${project.name.replace(/\s/g, '_')}_completo.csv`);
                link.style.visibility = 'hidden';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                toast({ title: "Projeto Exportado!", description: `O projeto '${project.name}' (completo) foi exportado para um arquivo CSV.` });
            }
        } catch (error) {
            console.error("Error exporting single project:", error);
            toast({ title: "Erro ao Exportar", description: "Não foi possível exportar o projeto selecionado.", variant: "destructive" });
        }
    };
                    ...
                    {projects.map(project => {
                       ...
                       const isCompleted = progress === 100;
                        ...
                        <Card key={project.id} className={`cursor-pointer hover:border-primary transition-all ${isCompleted ? 'border-green-500/50' : ''}`} onClick={() => router.push(`/clients/${clientId}/projects/${project.id}`)}>
                                ...
                                <div className="flex justify-between items-start">
                                    ...
                                    <DropdownMenu>
                                      ...
                                      <DropdownMenuContent onClick={(e) => e.stopPropagation()}>
                                        ...
                                        <DropdownMenuItem onClick={() => handleExportSingleProject(project)}>
                            ...
                            <CardFooter>
                                <MiniCountdown deadline={project.deadline?.toDate()} />
                            </CardFooter>
                        </Card>
                    )})}
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <Input id="deadline" name="deadline" type="date" defaultValue={currentProject?.deadline ? format(currentProject.deadline.toDate(), 'yyyy-MM-dd') : ''} />
                            </div>
                        </div>
